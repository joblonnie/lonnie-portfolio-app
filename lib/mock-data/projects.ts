...
export const projectsData: Project[] = [
  {
    ...
    structuralContributions: [
      {
        title: "5개 앱의 공통 로직 중복 개발 문제를 NX Monorepo 기반 통합 관리로 개선",
        summary: "NX Monorepo 도입을 통해 5개 앱 통합 관리로 빌드 시간 40% 단축",
        primaryCategory: "개발 생산성 향상",
        problemDescription: [
          "VIMS 기반의 5개 앱을 각 고객사별로 다르게 개발하면서 공통 로직의 중복 개발 발생",
          "버전 관리의 복잡성으로 인해 일관성 없는 개발 환경 조성",
          "같은 기능을 각 제품마다 다르게 구현하여 유지보수 비용 증가",
          "버그 수정 시 5개 저장소를 모두 수정해야 하는 비효율 반복",
        ],
        solutionDescription: [
          "NX Monorepo를 도입하여 5개 앱을 하나의 워크스페이스에서 통합 관리",
          "도메인 특화 모듈 패키지(monitoring, alarm, settings)와 shared 패키지(UI 컴포넌트, 유틸리티, API 클라이언트)를 중앙화",
          "NX의 Affected Commands를 활용하여 변경된 부분만 빌드하도록 최적화",
          "캐싱 전략을 통해 반복 빌드 시간을 40% 단축",
        ],
        reflection: [
          "대규모 프로젝트의 아키텍처를 설계하고 구축한 의미 있는 경험",
          "단순히 코드를 모으는 것이 아니라, 각 앱의 독립성을 유지하면서도 공통 코드를 효율적으로 관리할 수 있는 균형점을 찾는 것이 중요했음",
          "팀 전체의 개발 생산성을 향상시키는 인프라 구축 경험을 통해 기술적 리더십 역량 강화",
        ],
      ...
      },
      {
        ...
        summary: "Feature-Sliced Design 아키텍처 적용을 통해 도메인별 모듈 응집도 향상",
        primaryCategory: "개발 생산성 향상",
        problemDescription: [
          "각 앱 내에서 모니터링, 알람, 설정 등 다양한 비즈니스 도메인이 섞여있어 코드 구조가 복잡",
          "새로운 기능 추가 시 어디에 코드를 작성해야 할지 명확하지 않음",
          "기존 코드에 의도치 않은 영향을 주는 경우가 빈번하여 유지보수 어려움",
        ],
        solutionDescription: [
          "Feature-Sliced Design 아키텍처를 적용하여 비즈니스 도메인별로 features를 분리",
          "각 feature 내에서 UI, API, 상태관리를 응집도 높게 구성",
          "shared 레이어에는 공통 컴포넌트와 유틸리티를, entities 레이어에는 비즈니스 엔티티를 배치",
          "명확한 의존성 규칙을 수립하여 계층 간 참조 방향 통제",
        ],
        reflection: [
          "아키텍처 패턴을 실제 프로젝트에 적용하면서 이론과 실무의 차이를 경험",
          "완벽한 아키텍처보다는 팀의 상황과 프로젝트의 특성에 맞는 실용적인 구조를 찾는 것이 더 중요",
          "팀원들과 아키텍처에 대한 합의를 이끌어내는 과정에서 커뮤니케이션 역량 향상",
        ],
        technologies: ["Feature-Sliced Design", "Layered Architecture", "Domain-Driven Design"],
      },
      {
        title: "WebSocket 이미지 스트리밍의 메모리 누수를 Chrome DevTools 분석 및 최적화로 개선",
        summary: "Chrome DevTools Memory 탭 활용하여 메모리 누수 원인 분석 및 해결로 메모리 사용량 70% 감소",
        primaryCategory: "성능 최적화",
        problemDescription: [
          "실시간 모니터링 환경에서 지속적인 이미지 스트리밍으로 인해 메모리 누수 발생",
          "장시간 사용 시 브라우저 성능이 급격히 저하되는 문제",
          "Chrome DevTools의 Memory 탭에서 Heap Snapshot 분석 결과, ObjectURL이 해제되지 않고 계속 메모리에 쌓이는 것을 확인",
        ],
        solutionDescription: [
          "Chrome DevTools의 Memory 탭에서 Heap Snapshot을 찍어 메모리 사용 패턴 분석",
          "Blob → ObjectURL 변환 시 메모리 누수가 발생하는 지점 특정",
          "사용이 끝난 ObjectURL을 URL.revokeObjectURL()로 즉시 해제하는 로직 추가",
          "이미지 캐싱에 LRU 정책을 적용해 오래된 이미지를 주기적으로 정리하여 메모리 사용량 70% 감소",
        ],
        reflection: [
          "Chrome DevTools를 활용하여 메모리 누수의 근본 원인을 정확히 파악하고 개선한 경험",
          "단순히 증상을 완화하는 것이 아니라, 메모리 프로파일링을 통해 문제의 본질을 이해하고 체계적으로 접근",
          "성능 최적화 문제를 과학적으로 접근하는 방법을 체득",
        ],
      ...
      },
      {
        ...
        summary:
          "TanStack Query의 useInfiniteQuery 활용한 커서 기반 무한 스크롤 구현으로 초기 렌더링 지연 제거 및 실시간 알람 인지 UX 개선",
      ...
      {
        title: "알람 이미지 결함 위치 파악의 어려움을 원클릭 결함 찾기 기능 직접 제안 및 구현으로 개선",
        summary: "이미지 컨트롤러와 원클릭 결함 찾기 기능을 직접 제안·구현하여 검수 정확도와 사용자 경험 향상",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
      ...
      {
        title: "소켓 기반 동영상 검수 환경에서 커스텀 재생 컨트롤러 직접 구현으로 검수 효율성 개선",
        summary: "소켓 기반 이미지 스트리밍 환경에 맞는 커스텀 동영상 재생 컨트롤러 구현",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
          "사용자가 업로드한 동영상을 검수할 때 소켓으로 이미지를 받아서 보여주는 형태라 기존 비디오 플레이어를 사용할 수 없음",
          "원하는 시점을 찾기 어려워 동영상 검수 효율성 저하",
          "프레임 단위 제어와 썸네일 미리보기 기능 부재",
        ],
        solutionDescription: [
          "소켓 기반 이미지 스트리밍 환경에 맞는 커스텀 재생 컨트롤러를 직접 구현",
          "seekbar를 통해 원하는 시점으로 즉시 탐색이 가능하도록 구현",
          "Canvas API를 활용하여 프레임 단위 렌더링 구현",
          "썸네일 미리보기 기능을 추가하여 사용자가 원하는 시점을 빠르게 찾을 수 있도록 개선",
        ],
        reflection: [
          "제약 조건 속에서 창의적인 해결책을 찾아낸 경험",
          "기존 라이브러리를 사용할 수 없는 상황에서 직접 커스텀 컨트롤러를 구현하면서 웹 기술의 근본적인 원리를 깊이 이해",
          "문제 해결을 위한 기술적 도전과 학습의 즐거움을 느낀 프로젝트",
        ],
    ...
    structuralContributions: [
      {
        ...
        summary: "전역 에러 핸들링 구조 설계 및 API 에러 코드 체계 통일로 시스템 안정성 확보",
        primaryCategory: "성능 최적화",
        problemDescription: [
    ...
    structuralContributions: [
      {
        ...
        summary: "ECharts 기반 BaseChart 컴포넌트 개발 및 모듈화로 차트 재사용률 90% 달성",
        primaryCategory: "개발 생산성 향상",
        problemDescription: [
  ...
  },
  {
    ...
    structuralContributions: [
      ...
      },
      {
        ...
        summary: "Feature-based 모듈 구조로 코드 응집도 및 유지보수성 향상",
        primaryCategory: "개발 생산성 향상",
        problemDescription: [
  ...
  },
  {
    ...
    structuralContributions: [
      ...
      },
      {
        ...
        summary: "IntersectionObserver 기반 무한 스크롤로 대용량 데이터 렌더링 문제 해결",
        primaryCategory: "성능 최적화",
        problemDescription: [
