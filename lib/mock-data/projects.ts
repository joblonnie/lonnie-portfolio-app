...
export const projectsData: Project[] = [
  {
    ...
    image: "/vims-logo.svg",
    background:
      "산업용 AI 기반 검사·모니터링 시스템을 통합 관리하는 웹 플랫폼으로, 초기 설계부터 개발을 주도하고 고객사별 맞춤 요구와 VOC를 반영해 실시간 데이터 처리·시각화 기능을 구현한 프로젝트입니다. NX Monorepo 기반으로 5개 고객사 앱을 관리하며, 공통 기능은 @saige/vims 패키지로 분리하고 제네릭 타입을 활용해 고객사별 커스터마이징이 가능한 구조로 설계했습니다.",
    detailedDescription: {
      results: [
    ...
    structuralContributions: [
      {
        ...
        summary:
          "단일 VIMS 앱에서 출발해 고객사별 요구가 증가하는 과정에서 공통 기능이 중복되는 문제를 해결하기 위해 NX 기반 모노레포 아키텍처를 구축했습니다. VIMS 공통 기능을 @saige/vims 패키지로 분리하고, 제네릭 타입을 활용해 동일한 구조에서 result만 다르게 사용하거나, 앱 내로 가져와 커스터마이징이 가능한 구조로 설계했습니다. 이를 통해 5개 고객사 앱에서 반복 개발과 유지보수 비용을 크게 줄였습니다.",
        ...
        solutionDescription: [
          ...
          "VIMS 제품에서 공통적으로 사용하는 기능(모니터링, 알람, 설정, 공통 타입, API 등)을 @saige/vims 패키지로 통합",
          "제네릭 타입을 활용해 동일한 컴포넌트 구조에서 고객사별로 result 타입만 다르게 주입하는 방식으로 유연성 확보",
          "앱별로 필요한 커스터마이징은 공통 패키지에서 가져와 확장하는 구조로 설계, 중복 코드 최소화",
        ],
        reflection: [
          "단일 앱에서 여러 고객사 앱으로 확장되는 과정에서 공통 패키지 구조의 필요성을 명확히 이해함",
          "제네릭 타입을 활용해 '구조는 동일하되 데이터만 다른' 패턴을 효과적으로 적용하는 경험 확보",
      ...
      },
      {
        ...
        reflection: [
          "요구사항을 그대로 구현하는 수준을 넘어서, 사용자의 실제 업무 맥락을 이해하고 문제를 능동적으로 발견하며 해결책까지 제시한 경험",
  ...
  },
  {
    ...
    background:
      "신사업으로 시작된 프로젝트로, 2명의 개발자가 2–3주 안에 MVP를 만들어 고객사에 시연해야 하는 상황이었습니다. 짧은 기간 안에 결과를 내기 위해 VISION팀 시니어 개발자가 만든 MVVM 기반 상태 관리 라이브러리 X-view-model을 채택해 빠르게 개발을 진행했습니다. 초기 시연에서 UI 측면에서 안정적으로 시연을 마쳤고, 이후 고객 VOC를 반영하고 성능을 고도화하면서 계약을 유지·확장할 수 있었습니다. 이후 중대재해처벌법 강화 흐름에 따라 해당 사업이 회사의 전략 사업으로 집중되었고, 제품의 GS 인증 1등급 취득에 기여했습니다.",
    detailedDescription: {
      overview:
    ...
    structuralContributions: [
      {
        title: "2-3주 내 MVP 개발을 위한 X-view-model 상태 관리 적용",
        summary:
          "촉박한 일정에 맞춰 러닝 커브가 낮은 X-view-model을 채택하여 MVP를 완료하고, UI 이슈 없이 고객사 시연에 성공했습니다.",
        primaryCategory: "개발 생산성 향상",
        problemDescription: [
          "2명의 개발자가 2-3주 내에 MVP를 개발하여 고객사에 시연해야 하는 촉박한 일정",
          "새로운 상태 관리 라이브러리 학습에 시간을 투자할 여유가 없는 상황",
          "빠른 개발과 안정적인 품질을 동시에 달성해야 하는 도전",
        ],
        solutionDescription: [
          "VISION팀 시니어 개발자가 만든 X-view-model(MVVM 패턴) 라이브러리 채택",
          "러닝 커브가 낮고 빠르게 적용 가능한 점을 고려하여 기술 스택 결정",
          "MVVM 패턴으로 View와 비즈니스 로직을 명확히 분리하여 개발 효율성 확보",
          "2명이서 역할 분담을 명확히 하고 집중 개발하여 일정 내 완료",
        ],
        reflection: [
          "촉박한 일정에서 적절한 기술 스택 선택이 프로젝트 성공에 결정적이었음",
          "이후 VOC 반영과 GS 인증까지 이어진 장기 프로젝트의 시작점이 된 경험",
        ],
      ...
      },
      {
        ...
        summary:
          "Konva.js로 안전 이벤트 바운딩 박스를 구현하고, 화면 비율에 맞는 동적 좌표 계산으로 리사이징 환경에서도 정확한 표시를 보장했습니다.",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
          "AI 연구소에서 전달받는 바운딩 박스 좌표가 원본 이미지 기준으로 전달됨",
          "모니터링 화면의 크기가 원본과 다르기 때문에 좌표를 그대로 사용할 수 없는 상황",
          "브라우저 창 크기가 동적으로 변경될 때마다 바운딩 박스 위치가 어긋나는 문제",
          "안전모 미착용, 화재, 연기 등 이벤트 위치를 정확히 표시해야 하는 안전상 중요성",
        ],
        solutionDescription: [
          "Konva.js를 활용하여 캔버스 기반 바운딩 박스 오버레이 시스템 구현",
          "원본 이미지 대비 현재 화면 비율을 계산하여 좌표 변환 로직 개발",
          "ResizeObserver를 활용해 화면 크기 변경 감지 및 실시간 좌표 재계산",
          "이벤트 타입별(화재: 빨강, 연기: 주황, 안전모 미착용: 노랑) 색상 구분 적용",
        ],
        reflection: [
          "AI 연구소와의 데이터 인터페이스를 명확히 정의하는 것이 중요했음",
          "동적 환경에서의 좌표 계산은 사용자 경험에 직접적인 영향을 미침을 체감",
          "안전과 직결되는 기능이라 정확도에 특히 신경 쓴 작업",
        ],
        technologies: ["React", "TypeScript", "Konva.js", "ResizeObserver", "Canvas API"],
      },
      {
        title: "GS 인증 1등급 취득을 위한 에러 핸들링 체계 구축",
        summary: "Error Boundary 정의와 Toast 메시지 체계화로 GS 인증 1등급 취득에 기여했습니다.",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
          "MVP 단계에서 에러 처리가 각 컴포넌트별로 산발적으로 이루어져 일관성 부재",
          "GS 인증 1등급 취득을 위해 사용자 인터렉션에 따른 적절한 피드백 제공 필요",
          "같은 에러라도 페이지마다 다른 메시지가 표시되어 사용자 혼란 야기",
        ],
        solutionDescription: [
          "Error Boundary를 계층별로 정의하여 각 scope에서 적절한 에러를 catch하도록 설계",
          "사용자 인터렉션(버튼 클릭, 폼 제출 등)에 따른 Toast 메시지 체계적 추가",
          "백엔드 개발자와 협업하여 API 에러 코드 체계 정의 및 일관된 에러 처리 로직 구현",
        ],
        reflection: [
          "체계적인 에러 핸들링이 사용자 신뢰도와 제품 품질 인증에 직결됨을 체감",
          "GS 인증이라는 명확한 목표가 있어 우선순위를 정하고 집중 작업할 수 있었음",
        ],
        technologies: ["React", "TypeScript", "Error Boundary", "Toast Notification", "X-view-model (MVVM)"],
      },
      {
        title: "안전 이벤트 중요도 구분 부재를 중대/일반 알림 분류로 개선",
        summary:
          "중대/일반 알림 분류 시스템으로 화재, 추락 같은 중대 사고를 강조 표시하여 현장 대응 효율성을 높였습니다.",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
          "모든 안전 이벤트가 동일한 방식으로 알림 처리되어 중대 사고와 일반 이벤트 구분 어려움",
          "화재나 추락 같은 중대 사고와 단순 안전모 미착용이 동일한 우선순위로 표시",
        ],
        solutionDescription: [
          "안전 이벤트의 중요도에 따라 중대/일반 알림 분류 시스템 구현",
          "중대 알림은 화면 중앙에 빨간색 강조 표시, 소리 알림과 함께 고정 표시",
          "일반 알림은 Toast로 관리하여 중요도별 신속한 인지와 효율적 대응 지원",
        ],
        reflection: [
          "사용자의 실제 작업 환경을 이해하고 우선순위를 명확히 구분한 것이 핵심",
          "안전과 직결되는 기능이라는 책임감을 느끼며 개발한 경험",
        ],
        technologies: ["React", "TypeScript", "WebSocket", "Toast Notification", "X-view-model (MVVM)"],
      },
      {
        title: "고정된 모니터링 화면의 한계를 드래그 앤 드롭 기능으로 개선",
        summary:
          "기획에 없던 드래그 앤 드롭 기반 카메라 위치 조정 기능을 직접 제안하고 Konva.js로 구현하여 사용자 편의성을 향상시켰습니다.",
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
        ...
        reflection: [
          "기획에 없던 기능을 제안하고 구현하여 사용자 만족도를 높인 사례",
          "사용자의 실제 니즈를 파악하고 능동적으로 해결책을 제시한 경험",
        ],
        technologies: ["React", "TypeScript", "Konva.js", "Drag and Drop", "LocalStorage"],
      },
    ],
    role: "프론트엔드 개발",
    frontendDevelopers: 2,
