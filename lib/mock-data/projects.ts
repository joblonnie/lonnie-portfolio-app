    ...
    structuralContributions: [
      ...
      },
      {
        ...
        problemDescription: [
          ...
          "기능이 커질수록 컴포넌트·상태·API·유틸의 경계가 모호해짐",
          "도메인 간 의존 방향이 뒤엉켜 리팩토링 시 의도치 않은 영향 발생",
        ],
        solutionDescription: [
          "Feature-Sliced Design(FSD)을 도입하여 도메인 중심·계층화된 구조로 재정비",
      ...
      },
      {
        ...
        problemDescription: [
          "한 달 단위 알람 히스토리가 수십만 건까지 증가하면서 UI 지연과 스크롤 버벅임 발생",
          "페이지 기반 호출 방식에서 최신 순 데이터 구조로 인한 중복 호출 문제",
          "히스토리 탐색 중 새 알람 도착 시 인지 어려움",
        ],
        solutionDescription: [
          "TanStack Query의 useInfiniteQuery와 fetchNextPage를 활용한 커서 기반 무한 스크롤 도입",
          "마지막 조회 알람의 queryTime을 커서로 사용해 중복 없이 다음 페이지 로드",
      ...
      },
      {
        ...
        problemDescription: [
          ...
          "고해상도 이미지에서 작은 결함 발견이 어려움",
          "이미지 확대/축소 기능 부재로 정밀한 검수 불가능",
        ],
        solutionDescription: [
          "이미지 컨트롤러 다이얼로그로 확대·축소·이동 가능한 내비게이션 UI 구현",
          "사용자 플로우 기반 '원클릭 결함 찾기' 기능 제안 및 채택",
  ...
  },
  {
    ...
    structuralContributions: [
      ...
      },
      {
        ...
        primaryCategory: "사용자 경험 개선",
        problemDescription: [
          "GS 인증 1등급 취득을 위해 사용자 인터랙션에 따른 피드백 메시지가 필요",
          "입력 필드별 유효성 검사와 적절한 helperText 제공 체계가 부재",
          "에러 발생 시 사용자에게 명확한 피드백 없이 앱이 중단되는 문제",
        ],
        solutionDescription: [
          "react-hook-form과 zod를 도입하여 스키마 기반 유효성 검사 체계 구축",
          "비밀번호 정책, 공백 검증 등 정책에 따른 schema 정의 및 적용",
          "입력 상태에 따른 동적 helperText 표시로 사용자 가이드 제공",
