  ...
  projects: [
    {
      ...
      structuralContributions: [
        {
          title: "1. 다양한 고객사의 요구 사항을 효율적으로 반영할 수 있는 방법은 없을까?",
          solutionList: [
            {
        ...
        {
          title: "2. 동일한 구조의 store 구조를 갖는 각각의 앱의 중복 코드 발생",
          solutionList: [
            {
          ...
          solutionList: [
            {
              ...
              description:
                "ECharts 기반 시계열 그래프에서 오늘 데이터는 API로 1회만 요청, 이후 발생 데이터는 WebSocket 이벤트로 수신하여 TanStack Query의 setQueryData로 누적 업데이트하는 방식으로 불필요한 API 호출을 제거했습니다. 기존 setInterval 방식 대신 setTimeout과 명시적 연결 플래그를 사용하는 이벤트 중심 재연결 방식으로 개선하여 시스템 리소스 효율화하였습니다.",
            },
          ],
          achievementList: [
        ...
        },
        {
          ...
          solutionList: [
            {
              title: "영상 데이터 포맷 변경 및 메모리 최적화",
              description:
                "영상 스트리밍 데이터 형식을 base64에서 binary로 전환하고 메모리 사용량을 최적화하여 실시간 영상 품질을 개선하였습니다.",
            },
          ],
          achievementList: [
      ...
      structuralContributions: [
        {
          ...
          solutionList: [
            ...
            },
            {
              ...
              description:
                "사용자 VOC를 반영한 기능 고도화와 품질 표준 수립을 통해 GS 인증 1등급을 획득했습니다. 백엔드 개발자와 협업하여 API 에러 코드 체계를 정의하고, 프로덕트 디자이너와 협력하여 표시 범위(Global/Local) 및 UI 계층을 결정했습니다. 예상 가능한 사용자 오류 및 네트워크 이슈에 대해 인터랙티브하고 직관적인 피드백 제공 구조를 설계 및 적용했습니다.",
            },
          ],
          achievementList: [
        ...
        {
          title: "4. 대용량 이벤트 데이터 렌더링 최적화",
          solutionList: [
            {
              title: "Virtual Scrolling 및 에러 핸들링 시스템 구축",
              description:
                "Virtual Scrolling을 구현하여 대량의 안전 이벤트 데이터를 효율적으로 렌더링하고, 이벤트 클릭 시 필터링된 리포트 페이지 연동 기능을 구현했습니다. API 에러 코드별 메시지 매핑 체계와 계층화된 에러 표시 구조를 설계했습니다.",
            },
          ],
          achievementList: [
            "Virtual Scrolling으로 대량 데이터 렌더링 성능 최적화",
            "이벤트 상세 조회 워크플로우 구현으로 사용자 경험 향상",
            "체계적인 에러 핸들링으로 사용자 경험 지표 개선에 기여",
          ],
        },
      ],
    ...
    },
    {
      ...
      structuralContributions: [
        ...
        {
          title: "2. ECharts 기반 모듈화 차트 아키텍처 구현",
          solutionList: [
            {
              title: "재사용 가능한 차트 라이브러리 구축",
              description:
                "ECharts 기반의 BaseChart 컴포넌트를 중심으로 라인 차트, 파이 차트, 바 차트 등 다양한 차트 컴포넌트로 구성된 재사용 가능한 차트 라이브러리를 구축했습니다. 수율 대시보드, 리소스 모니터링, 검사기별 수율 차트 등 여러 페이지에서 공통 활용할 수 있도록 설계했습니다. 각 차트를 개별 컴포넌트로 분리하고 React.memo 및 React Query 캐싱을 적용해 차트 간 불필요한 리렌더링을 방지하고 UI 성능을 최적화했습니다.",
            },
          ],
          achievementList: [
            "공통 BaseChart 인터페이스로 일관된 차트 개발 경험 제공",
            "차트별 최적화된 렌더링 로직으로 대용량 데이터 처리 성능 향상",
            "개발 생산성 약 30% 향상 및 유지보수성 강화",
          ],
        },
        {
          title: "3. 실시간 데이터 관리 체계 구축 및 성능 최적화",
          solutionList: [
            {
              title: "TanStack Query 기반 효율적 데이터 관리",
              description:
                "리소스 모니터링 페이지에서는 React Query의 refetchInterval을 활용해 Prometheus 메트릭을 주기적으로 폴링하여 실시간 동기화를 수행했습니다. TanStack Query의 refetchInterval과 커스텀 훅을 결합한 관심사별 데이터 관리 체계를 구축했습니다. React Query 캐싱 전략을 통해 불필요한 API 호출을 최소화하며 성능을 최적화했습니다.",
            },
          ],
          achievementList: [
        ...
        {
          title: "4. 사내 디자인 시스템 발전 기여 및 개발 표준화 촉진",
          solutionList: [
            {
              title: "디자인 시스템 컨트리뷰터 및 컴포넌트 개발",
              description:
                "Saige Elements 초기 도입 시 발생한 버그 및 호환성 문제를 해결하고, 신규 컴포넌트 개발로 디자인 시스템 안정화에 기여했습니다. Storybook과 Chromatic을 활용해 프로덕트 디자이너와 협업하며 UI 품질을 높였습니다. 사내 디자인 시스템 Saige Elements 컨트리뷰터로 참여하여 컴포넌트 개발 및 프로젝트에 반영하여 재사용성과 유지보수성을 확보했습니다.",
            },
          ],
          achievementList: [
    ...
    },
    {
      ...
      structuralContributions: [
          ...
          solutionList: [
            {
              ...
              description:
                "평가 효율성과 정확도를 높이기 위해 3단 UI 구조(문단/문장/평가 폼)를 설계하고 단계별 UX 흐름을 구현했습니다. 윤리성 평가라는 복잡한 업무를 단순화하기 위해 3단 UI 구조(문단/문장/평가 폼)를 설계하고, 평가 기준 안내와 입력 제한을 통한 정확한 데이터 수집 체계를 구축했습니다. 비개발자 사용자들도 쉽게 이해하고 평가할 수 있도록 인터페이스의 정보 계층 구조 및 시각적 구분 체계를 수립했습니다.",
            },
          ],
          achievementList: [
        ...
        {
          title: "3. Feature-based 아키텍처 설계 및 상태 관리 최적화",
          solutionList: [
            {
              title: "모듈화된 프로젝트 구조 및 상태 관리 체계",
              description:
    ...
    },
    {
      ...
      structuralContributions: [
        ...
        {
          title: "3. 직관적 텍스트 라벨링 인터페이스 구현",
          solutionList: [
            {
              title: "드래그 기반 라벨링 시스템 및 시각적 피드백",
              description:
                "window.getSelection API를 활용해 드래그 기반 텍스트 선택 및 라벨링 기능을 구현하여 사용자 경험을 개선했습니다. window.getSelection API를 활용한 드래그 기반 텍스트 선택과 시각적 라벨링 시스템을 구현하여 사용자가 직관적으로 텍스트를 라벨링할 수 있는 인터페이스를 구축했습니다. 텍스트 드래그 범위를 인식하고 시각적 라벨링을 가능하게 하는 인터랙션을 구현했습니다.",
            },
          ],
          achievementList: [
        ...
        {
          title: "4. 대용량 데이터 렌더링 성능 최적화",
          solutionList: [
            {
              title: "무한 스크롤 및 점진적 로딩 시스템",
              description:
                "IntersectionObserver를 활용한 무한 스크롤을 구현하여 대용량 데이터 렌더링 성능 문제를 해결했습니다. IntersectionObserver를 활용한 무한 스크롤 구현으로 초기 30개 문장만 렌더링하고 점진적 로딩을 통해 대용량 데이터 처리 성능을 최적화했습니다. 초기 30개 문장만 렌더링하고 스크롤 시 점진적으로 로딩하여 빠른 초기 로딩과 작업 연속성을 보장했습니다.",
            },
          ],
          achievementList: [
