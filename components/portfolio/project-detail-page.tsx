...
export function ProjectDetailPage() {
  ...
  const handleProjectNavClick = (selectedProjectId: number) => {
    ...
    // 선택된 프로젝트가 화면에 보이도록 스크롤 조정
    setTimeout(() => {
      if (projectNavRef.current) {
        const selectedElement = projectNavRef.current.querySelector(`[data-project-id="${selectedProjectId}"]`)
        if (selectedElement) {
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-5xl mx-auto p-6 space-y-8">
        ...
        <div className="mb-8">
          ...
          <div className="mb-8" ref={projectNavRef}>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {mockPortfolioData.projects.map((proj) => (
                ...
                >
                  <img src={proj.image || "/placeholder.svg"} alt={proj.title} className="w-full h-12  rounded mb-2" />
                  <p className="text-xs font-medium text-gray-900 leading-tight">{proj.title}</p>
                </div>
              ))}
            </div>
          </div>
                  ...
                  {project.contributions.map((contribution, index) => (
                    <div key={index}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm font-bold text-lime-600">{contribution.percentage}%</span>
                      </div>
                      <Progress value={contribution.percentage} className="h-2 bg-gray-200">
                        <div
                          className="h-full transition-all duration-300 ease-in-out rounded-full bg-gray-500"
        ...
        {project.detailedDescription && project.detailedDescription.results && (
          <Card className="border-gray-200">
            <CardContent className="p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-6">주요 성과</h2>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">{contribution.title}</h3>
                    </div>
                  </div>
