          ...
          <section className="mb-12">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <AlertCircle className="h-5 w-5 text-mocha-500" />
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white m-0">문제 상황 또는 요구 사항</h2>
              </div>
              <div className="flex flex-wrap gap-2">
                {project.structuralContributions?.map((_, index) => (
                  <Badge
                ...
                {project.technicalContributions?.map((_, index) => (
                  <Badge key={`technical-${index}`} variant="outline" className="text-xs border-gray-300 text-gray-600">
                    기술적 해결
                  </Badge>
                ))}
              </div>
            ...
            <p className="text-gray-600 dark:text-gray-400 leading-relaxed mb-6">
              프로젝트 진행 과정에서 직면한 주요 문제 상황과 이를 해결하기 위한 접근 방법을 소개합니다.
            </p>
              ...
              {project.structuralContributions?.map((contribution, index) => (
                <Card key={index} className="bg-gray-50 dark:bg-gray-800/50 border-0 shadow-sm">
                  <CardContent className="p-4">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">{contribution.title}</h3>
                    <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{contribution.description}</p>
                    <div>
                      <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-3">해결 방법 및 성과</h4>
              ...
              {project.technicalContributions?.map((contribution, index) => (
                <Card key={index} className="bg-gray-50 dark:bg-gray-800/50 border-0 shadow-sm">
                  <CardContent className="p-4">
                    ...
                    <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{contribution.description}</p>
                    <div>
                      <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-3">기술적 해결책 및 성과</h4>
                      <ul className="space-y-2">
                        {contribution.achievements.map((achievement, achIdx) => (
                          <li key={achIdx} className="flex items-start gap-3 text-gray-700 dark:text-gray-300">
                            <CheckCircle className="h-4 w-4 text-mocha-500 mt-0.5 flex-shrink-0" />
                ...
                {project.projectPhases.map((phase, index) => (
                  <Card key={index} className="bg-gray-50 dark:bg-gray-800/50 border-0 shadow-sm">
                    <CardContent className="p-4">
                      ...
                      <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-4">{phase.description}</p>
          ...
          <section className="mb-12">
            ...
            <div className="space-y-6">
              ...
              {project.detailedDescription?.summary && (
                <Card className="bg-gray-50 dark:bg-gray-800/50 border-0 shadow-sm">
                  <CardContent className="p-4">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">핵심 성과</h3>
              ...
              <Card className="bg-gray-50 dark:bg-gray-800/50 border-0 shadow-sm">
                <CardContent className="p-4">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">얻은 것 / 배운 점</h3>
                  <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
                    {project.learning ||
